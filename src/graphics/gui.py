...
class gui:
# Nastavení
    drawing_canvas_size = 280
    compressed_canvas_size = 28
    default_brush_radius = 3
    default_brush_smoothness = 0.5
    brush_preview_size = 100
    ...
    def __init__(self, root):
        ...
        # Parametry štětce
        self.brush_radius = self.default_brush_radius
        self.brush_intensity = 1.0
        self.brush_smoothness = self.default_brush_smoothness
        ...
        self.big_canvas = tk.Canvas(left_frame, width=self.drawing_canvas_size, height=self.drawing_canvas_size, bg='black', highlightthickness=2, highlightbackground='gray')
        self.big_canvas.pack(pady=5)

        # === MNIST PREVIEW ===
        tk.Label(middle_frame, text="MNIST náhled (28x28)", font=("Arial", 10, "bold")).pack()
        self.preview_canvas = tk.Canvas(middle_frame, width=self.compressed_canvas_size*10, height=self.compressed_canvas_size*10, bg='black', highlightthickness=2, highlightbackground='gray')
        ...
        self.brush_preview_canvas = tk.Canvas(right_frame, width=self.brush_preview_size, height=self.brush_preview_size, bg='black', highlightthickness=1, highlightbackground='gray')
        ...
        self.status_var.set("Připraven - nakreslete číslici")
        ...
        self.big_img = Image.new("L", (self.drawing_canvas_size, self.drawing_canvas_size), 0)
    ...
    def update_brush_preview(self, *args):
        ...
        pixel_size = min(self.brush_preview_size // brush_size, 12)  # Max 12px na čtvereček
        if pixel_size < 4:
        ...
        offset_x = (self.brush_preview_size - total_width) // 2
        offset_y = (self.brush_preview_size - total_height) // 2
        ...
        self.brush_preview_canvas.create_text(
            self.brush_preview_size//2, self.brush_preview_size-10,
        ...
        for dx in range(-self.brush_radius, self.brush_radius + 1):
            for dy in range(-self.brush_radius, self.brush_radius + 1):
                ...
                if intensity > 0.01:  # Pouze pokud má štětec nějaký efekt
                    px = x + dx
                    py = y + dy
                    if 0 <= px < self.drawing_canvas_size and 0 <= py < self.drawing_canvas_size:
    ...
    def update_preview(self):
        ...
        small_img = self.big_img.resize((self.compressed_canvas_size, self.compressed_canvas_size), Image.BILINEAR)

        # vytvořit náhled zvětšený 10x
        preview_large = small_img.resize((self.compressed_canvas_size*10, self.compressed_canvas_size*10), Image.NEAREST)
...
Kreslící plátno: {self.drawing_canvas_size}x{self.drawing_canvas_size}px
